...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt9xx_v24", 0}, {}};
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
//static struct i2c_client_address_data addr_data = { .forces = forces,};
static struct i2c_board_info __initdata i2c_tpd = { I2C_BOARD_INFO("gt9xx_v24", (0xBA >> 1))};
static struct i2c_driver tpd_i2c_driver =
{
    .probe = tpd_i2c_probe,
    .remove = tpd_i2c_remove,
    .detect = tpd_i2c_detect,
    .driver.name = "gt9xx_v24",
    .id_table = tpd_i2c_id,
...
CHIP_TYPE_T gtp_chip_type = CHIP_TYPE_NONE;
#else
...
static struct proc_dir_entry *gt91xx_hall_proc = NULL;
...
int gtp_send_cfg_switch(int status)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ...
        {
            ret = gtp_i2c_write(i2c_client_point, config_hallclose, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
        }

        GTP_INFO("gtp_send_cfg_switch ret = %d\n",ret);
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#if GTP_HALL_CLOSE_CFG
    u8 cfg_info_group1_hallclose[] = CTP_CFG_GROUP1_HALLCLOSE;
#endif
...
#if GTP_HALL_CLOSE_CFG                             
	u8 *send_cfg_buf_hallclose[6] = {cfg_info_group1_hallclose, cfg_info_group1, cfg_info_group2,
                cfg_info_group3, cfg_info_group4, cfg_info_group5};
#endif
    ...
    memcpy(&config_hallclose[GTP_ADDR_LENGTH], send_cfg_buf_hallclose[0], cfg_len);
#endif
    ...
    config_hallclose[cfg_len] = (~check_sum) + 1;
#endif
...
reset_proc:
    ...
    else  if( CHIP_TYPE_NONE== gtp_chip_type)
    {
        return -1;
    }
#endif
...
#if GTP_HALL_CLOSE_CFG
static struct proc_dir_entry *gtp_wakeup_hall_state;
#define	GTP_HALL_SWITCH_FILE	"gt9xx_hall_switch"
static ssize_t gt9xx_hall_switch_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	return sprintf(buffer,"%d\n",gtp_hall_state);
}

static int tpd_clam_statue=0;
void tpd_nodify_clam_open(int isclamopen)
{
	if(i2c_client_point == NULL)
		return -EFAULT;

    if (gt91xx_hall_proc == NULL)
	    return -EFAULT;
	
	 tpd_clam_statue = isclamopen;
	 down(&g_hall_sem);
    
	printk("gtp_hall_state = %d", !tpd_clam_statue);
    gtp_send_cfg_switch(!tpd_clam_statue);
    msleep(300);
    up(&g_hall_sem);
    return SUCCESS;
}
static ssize_t gt9xx_hall_switch_write(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    char temp[25] = {0}; // for store special format cmd
    char mode_str[15] = {0};
   ...
   if(i2c_client_point == NULL)
    ...
    else if(strcmp(mode_str, "open") == 0)
    {
        gtp_hall_state = 1;
    }
    else
...
#if GTP_HALL_CLOSE_CFG
static const struct file_operations gt_hall_proc_fops = { 
    .write = gt9xx_hall_switch_write,
    .read = gt9xx_hall_switch_read
};
#endif

static struct tpd_driver_t tpd_device_driver;

s32 gtp_read_config_version(struct i2c_client *client, u8 *cfg_version)
{
     s32 ret=-1;
...
//#ifdef VELOCITY_CUSTOM
	...
	gt91xx_hall_proc = proc_create(GTP_HALL_SWITCH_FILE, 0666, NULL, &gt_hall_proc_fops);
	if (gt91xx_hall_proc == NULL)
	{
             GTP_ERROR("create_proc_entry %s failed\n", GTP_HALL_SWITCH_FILE);
	}
    ...
    ret = gtp_read_config_version(client, &cfg_version_info);

    if(SUCCESS== ret)
       sprintf(tpd_device_driver.descript,"Vno: %d", cfg_version_info);
...
static s8 gtp_enter_doze(struct i2c_client *client)
{
    ...
    GTP_DEBUG_FUNC();
    if(tpd_halt == 0)//add by hct wanghuadong
    {
	GTP_INFO("gtp_enter_doze tpd_halt=%d,gtp_wakeup_sleep",tpd_halt );
...
static void tpd_resume(struct early_suspend *h)
{
...
#if GTP_GESTURE_WAKEUP //add by hct wanghuadong
    doze_status = DOZE_DISABLED;
#endif
    ...
    queue_delayed_work(gtp_charger_check_workqueue, &gtp_charger_check_work, clk_tick_cnt);
			  ...
			  msleep(300);
			  {
				  tpd_nodify_clam_open(tpd_clam_statue);
			  }
#endif
}
#if GTP_GESTURE_WAKEUP
