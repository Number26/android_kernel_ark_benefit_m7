...
#include "tpd_custom_ili2115.h"

//define  IIC INTERFACE
#define  IIC_NUMBER  1
//define  power 
...
#endif
#ifdef GESTURE
#define GESTURE_LEFT			0x3c
#define GESTURE_RIGHT			0x3e
#define GESTURE_UP		    	0x5e
#define GESTURE_DOWN			0x64
...
static int ilitek_i2c_only_read(struct i2c_client *client,uint8_t *data, int length);
...
static int ilitek_i2c_process_and_report(void)
{
	int i, len, ret, x=0, y=0,key,org_x=0, org_y=0,mult_tp_id,j,packet = 0,tp_status = 0,release_flag[10]={0},buffer_flag[10] = {0},cmd_flag=0;
	unsigned char key_id=0,key_flag=1;
	static unsigned char last_id=0;
     #ifdef GESTURE
	int gesture_mode=0;
     #endif
	 unsigned char release_counter = 0,max_point=6;		
		...
		for(i = 0; i < max_point; i++)
		{
			...
			if(tp_status)
			{
				...
				if(touch_key_hold_press == 0){
					...
					input_event(input, EV_ABS, ABS_MT_PRESSURE, i2c.pressure[i]);
					input_mt_sync(input);
					release_flag[i] = 1;
					i2c.keyflag = 1;
				//	DBG("Point, ID=%02X, X=%04d, Y=%04d,pressure = %d,release_flag[%d]=%d,tp_status=%d,keyflag=%d\n",i, x,y,i2c.pressure[i],i,release_flag[i],tp_status,i2c.keyflag);	
				}
		...
		}
{
	...
	if(i2c.protocol_ver & 0x200)
	{
		...
		if(len == 0){
			...
			#endif
		}//len就是点的个数，len较大说明有防止丢点机制，这里是没点的时候的保险措施\E3\80?
		input_sync(input);
	}
	return 0;
}
...
static int ilitek_i2c_read_tp_info( void)
{
	...
	if((i2c.protocol_ver & 0xFF00) == 0x200){
		...
		//get key infotmation
                 //ILITEK_TP_CMD_GET_KEY_INFORMATION 0x22
		if(ilitek_i2c_read(i2c.client, 0x22, buf, 29) < 0){
			return -1;
		}
		if (i2c.keycount > 5){
		...
		for(i = 0; i < i2c.keycount; i++){
			...
			i2c.keyinfo[i].status = 0;
			printk(" key_id: %d, key_x: %d, key_y: %d, key_status: %d\n", i2c.keyinfo[i].id, i2c.keyinfo[i].x, i2c.keyinfo[i].y, i2c.keyinfo[i].status);
		}
	}
	return 0;
}
...
static int __init tpd_driver_init(void)
{
	...
	i2c_register_board_info(IIC_NUMBER, &i2c_tpd, 1);
