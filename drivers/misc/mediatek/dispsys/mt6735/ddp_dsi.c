...
//add by hct 2015.11.11
void DSI_Get_Current_CLK(DISP_MODULE_ENUM module, unsigned int *clk)
{
	int i=0;
...
#if defined(CONFIG_T89D_KLT_PROJ)||defined(CONFIG_T89N_TS_PROJ)||defined(CONFIG_T89B_OTD_PROJ)||defined(CONFIG_T25Y_HS_L422_PROJ)
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	 ...
	 if(read_data1.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0])
	{
			    ...
			    DISPDBG("[DSI]hct_add esd sucess\n");
				// clear rx data
			...
			else
			{
				...
				DISPDBG("[DSI]hct_add esd failed\n");
				DISPDBG("[DSI]enter cmp read_data0 byte1=0x%x lcm_esd_check_table[i].para_list[0]0x%x  \n",read_data0.byte1,dsi_params->lcm_esd_check_table[i].para_list[0]);
				ret = 1; // esd fail
				break;
			}
		}
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	else if(state == CMDQ_ESD_CHECK_CMP)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			//if(read_data0.byte1 == dsi_params->lcm_esd_check_table[i].para_list[0])
			if((read_data0.byte0 == dsi_params->lcm_esd_check_table[i].para_list[0])||(read_data0.byte1 == dsi_params->lcm_esd_check_table[i].para_list[0]))
			{
				// clear rx data
				// DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0);
				ret = 0; // esd pass
