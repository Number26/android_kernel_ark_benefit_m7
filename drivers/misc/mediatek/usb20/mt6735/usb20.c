...
#define FIRST_CONN_DELAY_CNT 40
			...
			DBG(0, "intval:%d, cnt:<%d,%d>\n", FIRST_CONN_DELAY_INTVAL, FIRST_TYPE_CHECK_CNT, FIRST_CONN_DELAY_CNT);
...
static ssize_t show_mobilesourceControl_value(struct device_driver *ddri, char *buf)
{
	printk("otg: status is %d.\n",mobile_source_flag);
	return scnprintf(buf, PAGE_SIZE, "%d\n", mobile_source_flag);
}

static ssize_t store_mobilesourceControl_value(struct device_driver *ddri, char *buf, size_t count)
{
	if(IS_MOBILE_SOURCE_OFF == mobile_source_flag)
	{
		printk("otg: is off %d, insert otg cable first!\n",mobile_source_flag);
		return count;
	}
    ...
    if(!strncmp(buf, "on", 2))
	{
		...
		printk("otg: is enable  %d\n",mobile_source_flag);
        mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN, GPIO_OUT_ONE);
	}	
	else
	{
		mobile_source_flag = IS_MOBILE_SOURCE_ON_DISABLE;
		printk("otg: is disable  %d\n",mobile_source_flag);
...
static DRIVER_ATTR(mobilesourceControl,   S_IWUSR | S_IRUGO, show_mobilesourceControl_value, store_mobilesourceControl_value);
#endif
...
#endif
#if defined(CONFIG_USB_MTK_OTG)
		driver_create_file(&mt_usb_driver.driver, &driver_attr_mobilesourceControl);
#endif
