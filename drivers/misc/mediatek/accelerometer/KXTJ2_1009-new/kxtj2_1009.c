...
#if 0//defined(CONFIG_T93_PROJ)  // imp: need close, because when call cancel_delayed_work_sync may timeout result phone can not resume normally  
#define ACC_ESD_PROTECT
#endif

#if  defined(ACC_ESD_PROTECT)
#define ACC_ESD_CHECK_CIRCLE        200
static struct delayed_work acc_esd_check_work;
static struct workqueue_struct *acc_esd_check_workqueue = NULL;
static void acc_esd_check_func(struct work_struct *);
#endif
#ifdef CUSTOM_KERNEL_SENSORHUB
...
#if defined(CONFIG_GSENSOR_ESD_CHECK_NEW_SUPPORT)
    int esd_ret = -1;
	u8 esd_buf[2] = {0};
    int esd_acc_power_status = -1;
#endif
...
#if defined(CONFIG_GSENSOR_ESD_CHECK_NEW_SUPPORT)
{
    ...
    esd_acc_power_status = hwmsen_read_block(obj_i2c_data->client, KXTJ2_1009_REG_POWER_CTL, esd_buf, 0x01);
    if (enable_status && !(esd_buf[0] >> 7)) {
       // mutex_lock(&kxtj2_1009_mutex);
    	esd_buf[0] |= KXTJ2_1009_MEASURE_MODE;
        esd_buf[1] = esd_buf[0];
        esd_buf[0] = KXTJ2_1009_REG_POWER_CTL;
        esd_ret = i2c_master_send(obj_i2c_data->client, esd_buf, 0x2);            
     //   mutex_unlock(&kxtj2_1009_mutex);
        printk("tek.xing acc_esd_check_func ---enable acc esd recovery \n");
    }
}
#endif
...
#if defined(ACC_ESD_PROTECT)
    if(enable == TRUE) {
        printk("tek.xing acc esd enable esd check\n");
        queue_delayed_work(acc_esd_check_workqueue, &acc_esd_check_work, ACC_ESD_CHECK_CIRCLE);       
    } else {
        printk("tek.xing acc esd disable esd check\n");
        cancel_delayed_work_sync(&acc_esd_check_work);
    }
#endif
	GSE_LOG("KXTJ2_1009_SetPowerMode %d!\n ",enable);
...
#if defined(ACC_ESD_PROTECT)
static void acc_esd_check_func(struct work_struct *work)
{
    int i;
    int ret = -1;
	u8 buf[2] = {0};
    int acc_power_status = -1;
    if (-1 == kxtj2_1009_init_flag) {
        return;
    }
    acc_power_status = hwmsen_read_block(obj_i2c_data->client, KXTJ2_1009_REG_POWER_CTL, buf, 0x01);
    if (enable_status && !(buf[0] >> 7)) {
        mutex_lock(&kxtj2_1009_mutex);
    	buf[0] |= KXTJ2_1009_MEASURE_MODE;
        buf[1] = buf[0];
        buf[0] = KXTJ2_1009_REG_POWER_CTL;
        ret = i2c_master_send(obj_i2c_data->client, buf, 0x2);            
        mutex_unlock(&kxtj2_1009_mutex);
        printk("tek.xing acc_esd_check_func ---enable acc esd recovery \n");
    }
        queue_delayed_work(acc_esd_check_workqueue, &acc_esd_check_work, ACC_ESD_CHECK_CIRCLE);
...
#if defined(ACC_ESD_PROTECT)
    INIT_DELAYED_WORK(&acc_esd_check_work, acc_esd_check_func);
    acc_esd_check_workqueue = create_workqueue("acc_esd_check");
    queue_delayed_work(acc_esd_check_workqueue, &acc_esd_check_work, ACC_ESD_CHECK_CIRCLE);
#endif
#if defined(CONFIG_HAS_EARLYSUSPEND) && defined(USE_EARLY_SUSPEND)
...
#if defined(ACC_ESD_PROTECT)
    destroy_workqueue(acc_esd_check_workqueue);
#endif
