...
static void hw_bc11_dump_register(void)
{
 ...
 kal_uint32  temp_cv_reg_val=0;
 kal_uint32  pre_cv_reg_val=0;
...
#define HIGH_NCP_CV_VALUE           0x2F    // BATTERY_VOLT_04_450000_V, onstep is 25mv
 ...
 static kal_uint32 charging_hw_init(void *data)
{
   //static kal_uint32 run_hw_init_once_flag=1;
    ...
    if(battery_voltage <HIGH_HIGH_CV_CHECK_VOLTAGE )
    ...
    else    
    {
        ...
        temp_cv_reg_val = pre_cv_reg_val;
            ...
            temp_cv_reg_val = 0x2C;   //BATTERY_VOLT_04_400000_V;
    }
    printk("Jay_ hw_set_ncp_cv=%x\n", temp_cv_reg_val);
...
//  ncp1854_set_int_mask(0x0); //disable all interrupt
   ...
   ncp1854_set_chgto_dis(0x1); //disable charge timer
...
#ifdef CONFIG_HCT_NCP_HIGHER_CV
                  ncp1854_set_ieoc(0x2); // terminate current = 150mA for ICS optimized suspend power
#else
                  ncp1854_set_ieoc(0x0); // terminate current = 100mA for ICS optimized suspend power
#endif
#else
  #if defined(CONFIG_HCT_NCP_HIGHER_CV)
	#if defined(CONFIG_T89D_KLT_PROJ)
         ncp1854_set_ieoc(0x7); // terminate current = 275mA for ICS optimized suspend power
	#else
         ncp1854_set_ieoc(0x4); // terminate current = 225mA for ICS optimized suspend power
	#endif
  #else
         ncp1854_set_ieoc(0x2); // terminate current = 200mA for ICS optimized suspend power
  #endif
	...
	//}
#ifdef DIS_AICL_EN
    	ncp1854_set_aicl_en(0x0); //enable AICL as PT team suggest
#else
...
#if defined (CONFIG_HIGH_4400_BATTERY_VOLTAGE_SUPPORT)
		cv_value = BATTERY_VOLT_04_425000_V;
#elif defined (HIGH_BATTERY_VOLTAGE_SUPPORT)
		cv_value = BATTERY_VOLT_04_375000_V;
#else
            cv_value = BATTERY_VOLT_04_225000_V;
#endif
...
#if defined(CONFIG_NCP1854_DUAL_SUPPORT)
    ncp1854_charging_set_cv_voltage_sub(register_value);
#endif
	return status;
 } 	
...
#if defined(CONFIG_NCP1854_DUAL_SUPPORT)
    *(kal_uint32 *)data += ncp1854_charging_get_current_sub();
 ...
 static kal_uint32 charging_set_current(void *data)
 {
	...
	//kal_uint32 current_high_flag = 0;
...
#if defined(CONFIG_NCP1854_DUAL_SUPPORT)
      kal_uint32 sub_current_value = 0;
      ...
      pr_notice("ncp: charging_set_current %d, sub=%d\n",current_value,sub_current_value);
#endif
...
#if defined(CONFIG_NCP1854_DUAL_SUPPORT)
       pr_notice("ncp: now set sub_current sub=%d\n",sub_current_value);


       if(sub_current_value)
            charging_set_current_sub(sub_current_value);
 ...
 static kal_uint32 charging_set_input_current(void *data)
 {
	...
	kal_uint32 current_value = *(kal_uint32 *)data;
          ...
          pr_notice("ncp: charging set_input_current %d,sub=%d\n",current_value,sub_current_value);
#endif
	...
	//ncp1854_set_iinset_pin_en(0x0); //Input current limit and AICL control by I2C
	    ...
	    ncp1854_set_iinlim_en(0x1); //enable input current limit
            ...
            ncp1854_set_iinlim_en(0x0); //enable input current limit
	//ncp1854_set_aicl_en(0x0); //disable AICL
...
static kal_uint32 charging_hw_init_dual(void *data)
{
...
static kal_uint32 charging_dump_register_dual(void *data)
{
...
extern kal_uint32 ncp1854_charging_enable_sub(void *data);
static kal_uint32 charging_enable_dual(void *data)
{
