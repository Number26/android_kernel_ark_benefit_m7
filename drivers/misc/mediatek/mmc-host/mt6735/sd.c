...
#if defined(CONFIG_HCT_SDCARD_DEBOUNCE)||defined(CONFIG_T99_PROJ)
unsigned int hct_msdc1_cd_irq= 0xFFFF;
#endif
...
#if 0  //hct:jay modify from 35a, use this some T card switch voltage failed when resume
...
#if 1  //add by hct from mtk FAQ15216
    if(!(host->hw->cd_level ^ host->sd_cd_polarity)){
        printk("1_power_off_first\n");
        pmic_config_interface_nolock (0x0A20,0x0,0x1,1); //add here for disable VMC
...
#if defined(CONFIG_HCT_SDCARD_DEBOUNCE)||defined(CONFIG_T99_PROJ)
	#if defined(CONFIG_T99C_JQ_PROJ)||defined(CONFIG_T99L_DWS_PROJ)||defined(CONFIG_T991_PROJ)||defined(CONFIG_T99F_LF_PROJ)   
	 debounce = 400000;
	#else
	 debounce = 1000000;
	#endif
...
#if 0  //add by hct from mtk FAQ15216
