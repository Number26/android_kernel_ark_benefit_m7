...
static struct hct_hall_struct hct_hall_data;
#endif
...
#ifdef CONFIG_LETTICE_VD_SUPPORT
#define VOICE_WAKUP_KEY                                 KEY_CHAT
void voicewakup_report()
{
       printk("voicewakup_report\n");
       input_report_key(kpd_input_dev, VOICE_WAKUP_KEY, 1);
       input_report_key(kpd_input_dev, VOICE_WAKUP_KEY, 0);
       input_sync(kpd_input_dev);
}
#endif
...
static void hct_hall_init_work_func(struct work_struct *work)
{
	...
	mt_eint_set_hw_debounce(CUST_EINT_KPD_SLIDE_NUM, 20);
	mt_eint_registration(CUST_EINT_KPD_SLIDE_NUM, EINTF_TRIGGER_LOW, hct_hall_eint_handler, 1);
}
...
{
	int slid = mt_get_gpio_in(GPIO_QWERTYSLIDE_EINT_PIN);;
...
static void hct_hall_eint_handler(void)
{
    ...
    mt_eint_set_polarity(CUST_EINT_KPD_SLIDE_NUM, hct_hall_state);
    schedule_work(&(hct_hall_data.eint_work));

	printk("[hct-hall]:---interrupt\n");
}
#endif
/*********************************************************************/
...
#ifdef CONFIG_LETTICE_VD_SUPPORT
    __set_bit(VOICE_WAKUP_KEY, kpd_input_dev->keybit);
#endif
...
#ifdef CONFIG_HCT_HALL_SUPPORT
    INIT_WORK(&hct_hall_data.eint_work, hct_hall_eint_work_func);
	INIT_DELAYED_WORK(&hct_hall_data.init_work, hct_hall_init_work_func);
	schedule_delayed_work(&hct_hall_data.init_work, msecs_to_jiffies(500));
#endif
