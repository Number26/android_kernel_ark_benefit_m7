...
#if defined(__arm__)
	elf_core_copy_kernel_regs((elf_gregset_t *)&crash_record->arm32_cpu_regs[cpu], ptregs);
#else
...
#if defined(__arm__)
	elf_core_copy_kernel_regs((elf_gregset_t *)&crash_record->arm32_cpu_regs[cpu], &regs);
#else
	elf_core_copy_kernel_regs((elf_gregset_t *)&crash_record->aarch64_cpu_regs[cpu], &regs);
#endif
	crash_save_cpu((struct pt_regs *)&regs, cpu);
...
#error "Unsupport archtecture"
...
#elif (defined(CONFIG_ARM_PSCI) || defined(CONFIG_MTK_PSCI)) && defined(CONFIG_TRUSTONIC_TEE_SUPPORT)
	{
		...
		for (i = 0; i < CONFIG_NR_CPUS; i++) {
			crash_save_cpu((struct pt_regs *)&crash_record->arm32_cpu_regs[cpu], cpu);
		}
	}
